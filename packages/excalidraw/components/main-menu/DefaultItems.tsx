...
const PreferencesToggleAIShapeRecognitionItem = () => {
  const { t } = useI18n();
  const actionManager = useExcalidrawActionManager();
  const appState = useUIAppState();
  return (
    <DropdownMenuItemCheckbox
      checked={appState.aiShapeRecognitionEnabled}
      onSelect={(event) => {
        actionManager.executeAction(actionToggleAIShapeRecognition);
        event.preventDefault();
      }}
    >
      {t("buttons.aiShapeRecognition")}
    </DropdownMenuItemCheckbox>
  );
};
