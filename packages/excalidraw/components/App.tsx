...
class App extends React.Component<AppProps, AppState> {
  ...
  private processAIRecognitionBatch = async () => {
    ...
    this.aiPendingElements = [];
    this.aiRecognitionTimer = null;
  ...
  /**
   * Apply a recognized shape — delete freedraw elements, insert shape element.
   */
  private applyShapeRecognition = (
    shape: import("../ai").RecognizedShape,
    ...
    if (shape.type === "line" || shape.type === "arrow") {
      const startX = shape.startPoint?.x ?? bounds.x;
      const startY = shape.startPoint?.y ?? bounds.y;
      const endX = shape.endPoint?.x ?? bounds.x + bounds.width;
      const endY = shape.endPoint?.y ?? bounds.y + bounds.height;

      if (shape.type === "arrow") {
        replacementElement = newArrowElement({
          type: "arrow",
          x: startX,
          y: startY,
        ...
        replacementElement = newLinearElement({
          type: "line",
          x: startX,
          y: startY,
          strokeColor: firstEl.strokeColor,
      ...
      this.scene.mutateElement(replacementElement, {
        width: bounds.width,
        height: bounds.height,
      });
    }
